...
export const ProfilePage = () => {
    ...
    const handleSave = () => {
        if (user?.id) {
            // Формируем объект только из измененных полей
            const dataToSend: any = {};
            if (editData.username !== user.username) dataToSend.username = editData.username;
            if (editData.password) dataToSend.password = editData.password;

            if (Object.keys(dataToSend).length > 0) {
                dispatch(updateUserProfile({ id: user.id, data: dataToSend }))
                    .unwrap()
                    .then(() => {
                        setIsEditing(false);
